...
def search_text(query, content):
    ...
    global name
    global hobby
    url = "https://aip.baidubce.com/rpc/2.0/nlp/v1/txt_monet?access_token=24.e5c0636d07945ac688bcc55b383266c5.2592000.1701500350.282335-42226703"
    payload = json.dumps({
        "content_list": [
            {
                "content": content,
...
def follow22(parameter):
    ...
    #获取当前位置
        ...
        if(orientation!=None):#当前朝向信息获取后
            ...
            body_segment_response = body_segment()
            # print(body_segment_response.result)
     
            if(body_segment_response.result=="success"):
                target_angle = current_angle
                # print(body_segment_response.cX)
                # print(body_segment_response.cY)
                mean_distance=body_segment_response.mean_distance
                ...
                if(body_segment_response.cX>340):#角度计算,如果当前人体相对于摄像头偏右,向右转,角度变小一点
                    ...
                    target_angle=current_angle+math.degrees(math.atan((320-body_segment_response.cX)/l))
                # print("xianzaiweizhi the position is:/n")
                # print([position,current_angle])
                # nonzero_values = data_array[data_array != 0]
                # mean_distance = np.median(nonzero_values)
                print("mean_distance:",mean_distance)
                if(mean_distance>dzhiding):
                    target_position_x = position.x+(mean_distance-dzhiding)*math.cos(math.radians(target_angle))
                    target_position_y = position.y+(mean_distance-dzhiding)*math.sin(math.radians(target_angle))
                    target_position_z = 0 
                else:
                    target_position_x = position.x
                    target_position_y = position.y
            ...
            while(1):#当成功到达门口后
                ...
                if(idaoda>2):
                    if((navigator.is_goal_reached())):
                        break
                print("angle:",navigator.is_goal_reached())
            ...
            while(1):#当成功到达门口后
                ...
                if(idaoda>2):
                    if((navigator.is_goal_reached())):
                        break
                print("line:",navigator.is_goal_reached())
            # rospy.sleep(0.1)
...
def follow(paramater):
    ...
    # while(True):
    #     body_segment_response = body_segment()
    #     print("mean_distance:",body_segment_response.mean_distance)
    while(True):  
    #获取当前位置
        orientation,position=navigator.get_current_position()
        if(orientation!=None):#当前朝向信息获取后
            ...
            if(body_segment_response.result=="success"):
                ...
                while(not(navigator.is_goal_reached())):#当成功到达后
                    rospy.sleep(0.2)
                    print("turn_angle",navigator.is_goal_reached())
...
class Navigate_sofa_desk(smach.State):
    ...
    def execute(self, userdata):
        ...
        if(destination=="sofa"):
            ...
            print(navigator.is_goal_reached())
            rospy.sleep(2)
            current_position = sofa_position
        else:
            navigator.navigate_to_point(chair_position)
            rospy.sleep(2)
            print(navigator.is_goal_reached())
            rospy.sleep(2)
            current_position = chair_position
        #同时也可以显示一些导航到哪个位置的一些内容
        while(not navigator.is_goal_reached()):
            navigator.navigate_to_point(current_position)
            rospy.sleep(2)
            print(navigator.is_goal_reached())
            rospy.sleep(4)
...
class Introduce(smach.State):
    ...
    def execute(self, userdata):
        ...
        play_wav("其他帮助.wav")
        play_wav("gpt多功能.wav")
        text_org="我需要你帮助我对以下指令内容进行提取，并提取出其中的导航任务,人脸检测与识别任务,播放音乐任务,聊天任务,跟踪任务以及相应任务的参数,然后将其按照我向你输入的答案输出格式为我输出。以下是一些案例:注意一定要识别其中的动作在逻辑上的先后次序以及相应动作的参数.请带我前往沙发    答案:动作1:导航 参数1:沙发    陪我聊聊天气    答案:动作1:聊天 参数1:天气    请跟着我走    答案:动作1:跟踪 参数1:我    请先放一首歌演员,然后跟我走    答案:动作1:播放音乐  参数1:演员  动作2:跟踪  参数2:我    请带我去厨房寻找金东涛  答案:动作1:导航 参数1:沙发 动作2:检测识别 参数2:金东涛  请去学校找陶沐村,但在此之前现在原地寻找金东涛 答案:动作1:导航 参数1:原地 动作2:检测识别 参数2:金东涛 动作3:导航 参数3:学校 动作4:检测识别 参数4:陶沐村   请去沙发放薛之谦的丑八怪 答案:动作1:导航 参数1:沙发 动作2:播放音乐 参数2:丑八怪   去椅子上找瓶水  答案:动作1:导航 参数1:椅子 动作2:检测识别 参数2:饮料。现在，我将输入我的指令。"
        voice_words_subscriber()    
        # 运行rosrun命令
        iat_publish_process = subprocess.Popen(["rosrun", "robot_voice", "iat_publish"])
        print("start success")
